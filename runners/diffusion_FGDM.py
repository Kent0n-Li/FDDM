...
class TrainDataset_FDDM(data.Dataset):
    ...
    def __getitem__(self, item):
        ...
        #print(imagename)
        npimg = cv2.imread(imagename, cv2.IMREAD_GRAYSCALE)
        ...
        threshold_random = random.random() * 250
        ...
        #print(nplabs.max(), threshold_random)
...
class Dataset_FDDM_test(data.Dataset):
    def __init__(self, path_low, path_high, size=None, test=None, fraction=1, rank=0):
        ...
        print(f"total_num: {total_num}, fraction: {fraction}, rank: {rank}")
...
class FDDM(object):
    ...
    def sample_fid(self, model,data_loader):
        ...
        total_noise_levels = self.args.total_noise_levels
        high_level= self.args.high_level
        save_name = 'ddpm_image_samples_'+str(total_noise_levels)+'_'+str(high_level)
        ...
        with torch.no_grad():
            for batch, (x,edge,x_name) in enumerate(data_loader):
                x_name = x_name[0]
                exited_result_list = os.listdir(os.path.join(self.args.exp, save_name, 'result'))
                if x_name in exited_result_list:
                    print(f"skip {x_name}")
                    continue
                ...
                x = self.sample_image(x, model, edge, total_noise_levels, high_level)
    ...
    def sample_image(self, x, model, edge, total_noise_levels, high_level ,last=True):
        try:
            skip = self.args.skip
        except Exception:
            skip = 1
